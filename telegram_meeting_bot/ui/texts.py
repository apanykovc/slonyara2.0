...
def show_help_text() -> str:
    return (
        "❓ *Справка*\n\n"
        "Я напоминаю о встречах и веду список всех задач.\n"
        "Ниже — как работает бот и где что искать.\n\n"
        "*Как создать напоминание*\n"
        "1. Отправьте строку формата `ДД.ММ ТИП ЧЧ:ММ ПЕРЕГ НОМЕР`.\n"
        "2. Получите подтверждение с датой напоминания.\n"
        "3. В личке можно выбрать чат, куда отправить встречу.\n\n"
        "*Полезные факты*\n"
        "• Пример сообщения: `08.08 МТС 20:40 2в 88634`.\n"
        "• Поддерживаются ссылки t.me/c/…, @username и числовые chat_id.\n"
        "• Команда `/register` в чате добавляет его в список доступных.\n"
        "• Таймзона и оффсет настраиваются через ⚙️ *Настройки*.\n\n"
        "*Кнопки у поля ввода*\n"
        "• «Активные» — мгновенно покажет весь список напоминаний.\n"
        "• «Справка» — откроет эту подсказку.\n"
        "Каждое новое нажатие отменяет предыдущий ввод и запускает свежий сценарий.\n\n"
        "*Управление встречами*\n"
        "• Нажмите ⚙️ рядом с задачей, чтобы перенести, отменить или отправить её сейчас.\n"
        "• Для повторов используйте кнопку 🔁 в карточке встречи.\n"
    )
